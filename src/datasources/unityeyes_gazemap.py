...
import util.gaze
import util.gazemap_uteyes


class UnityEyes_GazeMap(BaseDataSource):
    ...
    def __init__(self,
        ...
        self._file_stems = sorted([p[:-5] for p in os.listdir(unityeyes_path)  # [:-5] remove .json
                                   if p.endswith('.json')])
    ...
    @property              #The property decorator is responsible for turning a method into a property call.
    ...
    def set_augmentation_range(self, augmentation_type, easy_value, hard_value):
        ...
        assert isinstance(augmentation_type, str)      # Make sure the augmentation_type parameter is str type
    ...
    def preprocess_entry(self, entry):
        ...
        def process_coords(coords_list):
            coords = [eval(l) for l in coords_list]      # eval(): str Conversion to List
        ...
        landmarks = np.concatenate([interior_landmarks[:, :2],  # 16
                                    iris_landmarks[:, :2],  # 32
                                    ])  # 48 in total
        landmarks = landmarks - [top_l_x, top_l_y]

        entry['gazemaps'] = util.gazemap_uteyes.from_gaze2d(
            landmarks, intput_size=(int(eye_height), int(eye_width)), output_size=(oh, ow), scale=0.5,
        ).astype(np.float32)
    ...
    def _preprocess_entry(self, entry):
        ...
        def value_from_type(augmentation_type):
            ...
            value = (np.clip(value, easy_value, hard_value)            # Intercept (min,max)
