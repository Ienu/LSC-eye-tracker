...
'''
    ...
    v6.0[insfan][4/25/2019]:Generate face, face_mask, eyes, gaze_point data to .npz file
'''
import numpy as np
import cv2
import time
...
def generate_face_eyes_roi(face_landmarks, src_img):
    ...
    # get eyes roi
    eye_width = 1.5 * max(abs(face_landmarks[0] - face_landmarks[2]), abs(face_landmarks[4] - face_landmarks[6]))
...
def main():
    ...
    amount = 0
    index = 0
    save_name = 'MPIIFaceGaze_fem_p00.npz'
    ...
    mask_data = np.zeros([amount, mask_size, mask_size, 1], dtype='float32')
    ...
    right_eye_data = np.zeros([amount 1, eyes_size[0], eyes_size[1], 3], dtype='uint8')
