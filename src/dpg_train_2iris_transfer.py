...
"""v0.0[insfan][5/25/2019] Main script for training the DPG_2Iris_Transfer (hourglass moudle use pre-trained weights) model for within-MPIIGaze evaluations"""
...
if __name__ == '__main__':
    ...
    for i in range(0, 1):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as session:
            ...
            # Define model
            from models import DPG_2Iris_Transfer
            model = DPG_2Iris_Transfer(
                session,
                learning_schedule=[
                    {
